...
function App() {
  ...
  const handleBridgeTransaction = useCallback(async (contract, destChainId, recipient, amount) => {
  ...
  }, [chainId, balances, handleFetchTokenBalance, handleFetchBalanceForChain]);
  ...
  const handleBridge = useCallback(async (amount, recipient) => {
    ...
    if (Number(chainId) !== Number(sourceChainId)) {
      try {
        ...
        return handleBridgeTransaction(newContract, destChainId, recipient || address, amount);
      } catch (error) {
        console.error('Network switch error:', error);
        return;
      }
      ...
      return handleBridgeTransaction(tokenContract, destChainId, recipient || address, amount);
    }
  }, [isConnected, tokenContract, sourceNetwork, destNetwork, networks, chainId, address, handleBridgeTransaction]);
